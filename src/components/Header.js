...
export const Header = ({ searchParam, setSearchParam }) => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        const searchParam = e.target.search.value;
        setSearchParam(searchParam);
        e.target.reset();

        return navigate(`/search?q=${searchParam}`);
    } 

    return (
        <header>
            <nav className="bg-white border-gray-200 dark:bg-gray-900">
                <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4 border-b-2 border-gray-200 dark:border-b-1 dark:border-gray-900">
                    <Link to="/" className="flex items-center">
                        <img src={Logo} className="h-8 mr-3" alt="Moviepedia Logo" />
                        <span className="self-center text-2xl font-semibold whitespace-nowrap dark:text-white">Moviepedia</span>
                    </Link>
